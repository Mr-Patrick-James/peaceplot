...
$title   = !empty($titleParts) ? implode(' · ', $titleParts) . ' — Block Summary' : 'Block Summary Report';
...
// Apply filters
$filtered = array_filter($blocks, function($b) use ($search, $hasOccupied, $hasVacant, $selBlocks) {
    if ($search && stripos($b['block'], $search) === false) return false;
    if ($hasOccupied && $b['occupied'] == 0) return false;
    if ($hasVacant   && $b['vacant']   == 0) return false;
    if (!empty($selBlocks) && !in_array(strtolower($b['block']), $selBlocks)) return false;
    return true;
});
...
<head>
  <meta charset="UTF-8">
  <title>Block Summary Report</title>
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .sub-row td { background:#f0f9ff !important; font-size:10px; color:#475569; padding:8px 12px 8px 28px; }
...
<div id="toolbar">
  ...
  <span class="meta"><?php echo $total; ?> block<?php echo $total !== 1 ? 's' : ''; ?></span>
...
<?php
$perPage = 30;
// Build flat rows (block + its sections)
$allRows = [];
    ...
    <table>
      ...
      <tbody>
        <?php foreach ($rows as $row): ?>
          <?php if ($row['type'] === 'block'): $b = $row['data']; ?>
            <tr style="font-weight:600;">
              <td><?php echo htmlspecialchars($b['block']); ?></td>
              <td align="right"><?php echo $b['section_count']; ?></td>
              <td align="right"><?php echo $b['total_lots']; ?></td>
              <td align="right" style="color:#f97316;"><?php echo $b['occupied']; ?></td>
              <td align="right" style="color:#10b981;"><?php echo $b['vacant']; ?></td>
            </tr>
          <?php else: $s = $row['data']; ?>
            <tr class="sub-row">
